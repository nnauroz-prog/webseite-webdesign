...
/**
 * Floating WhatsApp call-to-action — bottom-right corner, fixed
 * across all pages of the site. Renders only when the customer
 * provided a number. Pre-fills the chat with their configured
 * greeting so visitors don't have to think about what to write.
 */
export function SiteWhatsappButton({ website }: { website: WebsiteRow }) {
  const raw = website.whatsapp_number?.trim();
  if (!raw) return null;
  ...
  const message =
    website.whatsapp_message?.trim() ||
    `Hallo ${website.business_name}, ich habe eine Frage:`;
  ...
  return (
    <a
      ...
      aria-label={`WhatsApp-Chat mit ${website.business_name}`}
      className="fixed right-4 bottom-4 z-40 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-xl ring-1 ring-black/10 transition-all duration-200 hover:scale-110 hover:shadow-2xl sm:right-6 sm:bottom-6"
    >
      <svg
        viewBox="0 0 32 32"
        fill="currentColor"
